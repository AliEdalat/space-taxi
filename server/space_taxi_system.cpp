...
void taxi_system::accept_trip_request(string driver_username,string passenger_username){
	if(find_username(driver_username) && find_user(driver_username)->get_is_login() && find_username(passenger_username) && find_user(driver_username)->get_type() == "driver" && find_user(driver_username)->get_is_accepted()){
		...
		if(driver->find_trip_for_accept()){
			...
			result_line=" ";
		}else{cout<<"you do not have trip for acceptting!"<<endl; result_line= "you do not have trip for acceptting!";}
...
void taxi_system::determine_excellent_passengers(Date_time* date_time){
	...
	{	
		...
		if(!passengers[i]->get_is_excellent_passenger() && passengers[i]->get_num_of_trips() >= 3 && passengers[i]->get_gap_between_last_trips() < "1-00:00:00"){
...
void taxi_system::system_report_with_date(string from_date,string to_date){
	...
	{
		if(from_date <= date_time->get_date_time() && to_date <= date_time->get_date_time() && from_date <= to_date){
			cout<<credit<<' '<<number_of_trips(from_date,to_date)<<' '<<number_of_drivers(from_date,to_date)<<' '<<number_of_passengers(from_date,to_date)<<endl;
		}else{
			cout<<"from_date_time "<<from_date<<" and "<<to_date<<" are not valid date_times!"<<endl;	
		}
	}else{
